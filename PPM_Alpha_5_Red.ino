...
void scaleSetup(){
  ...
  Serial.print("Paddle Zero offset: ");
  Serial.println(myScale.getZeroOffset());
  Serial.print("Paddle calibration factor: ");
...
void calibrateScale(void){
  Serial.println();
  Serial.println(F("Paddle calibration"));

  Serial.println(F("Setup paddle with no weight on it. Press a key when ready."));
  ...
  Serial.println(F("Place known weight on paddle. Press a key when weight is in place and stable."));
  while (Serial.available()) Serial.read(); //Clear anything in RX buffer
  while (Serial.available() == 0) delay(10); //Wait for user to press key

  Serial.println(F("Please enter the weight, without units, currently sitting on the paddle (for example '4.25'): "));
  while (Serial.available()) Serial.read(); //Clear anything in RX buffer
  while (Serial.available() == 0) delay(10); //Wait for user to press key
  ...
  Serial.print(F("New Paddle Reading: "));
...
void scaleStuff(void){
  if (myScale.available()){
    ...
    if(settingsDetected == false){
      Serial.println("Paddle not calibrated. Enter 'c'.");
      if (Serial.available()){
    ...
    if (Serial.available()){
      ...
      if (incoming == 't'){ //Tare the scale
        Serial.println("Taring Paddle");
        myScale.calculateZeroOffset();
        Serial.print(F("New zero offset: "));
        Serial.println(myScale.getZeroOffset());
      } 
      else if (incoming == 'c'){ //Calibrate
        Serial.println("Calibrating Paddle");
        calibrateScale();
      }
    }
  }
...
void imuStuff(void){
  ...
  if (imuData == ALL_DATA_READY){
    ...
    strokePower = ((cAngularVelocity / 360 ) * 11 * currentWeight); //Calculate instantaneous power (Angular Velocity / 360 * Radius (reprogram to dynamically calculate) * Load)
    ...
    if((strokePower > 0) && (spm > 0)){ //Only look at positive power and stroke rates
      ...
      balance = 99; //testing L/R power balance... implement using a different IMU
